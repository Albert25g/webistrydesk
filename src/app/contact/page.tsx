...
function EmailButton({ addr, onCopy }: { addr: string; onCopy: (addr: string) => void }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl bg-gray-800/70 border border-white/10 px-4 py-3">
      <a href={`mailto:${addr}`} className="truncate underline">{addr}</a>
      <button
        type="button"
        onClick={() => onCopy(addr)}
      ...
      <section className="px-6 py-16">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-gray-300">
            Prefer email? Reach us directly — we typically reply within 1 business day.
          </p>
        </div>
        ...
        <div className="mx-auto mt-10 max-w-3xl grid gap-4">
          ...
          {copied && <p className="text-center text-sm text-green-400">Copied {copied}</p>}
        </div>

        {/* Divider */}
        <div className="mx-auto my-12 max-w-3xl text-center text-gray-400">— or —</div>
          ...
          <form onSubmit={onSubmit} className="grid gap-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input id="name" name="name" required className="w-full rounded-xl bg-gray-800/70 border border-white/10 px-4 py-3 outline-none focus:border-cyan-400" />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm text-gray-300 mb-2">Email *</label>
                <input id="email" name="email" type="email" required className="w-full rounded-xl bg-gray-800/70 border border-white/10 px-4 py-3 outline-none focus:border-cyan-400" />
              </div>
            </div>

            <div>
              <label htmlFor="message" className="block text-sm text-gray-300 mb-2">Message *</label>
              <textarea id="message" name="message" required rows={6} className="w-full rounded-xl bg-gray-800/70 border border-white/10 px-4 py-3 outline-none focus:border-cyan-400" placeholder="What can we help with?"></textarea>
            </div>
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 font-semibold hover:opacity-90 transition disabled:opacity-50"
              >
            ...
            <p className="text-xs text-gray-500 mt-2">
              ...
              <a className="underline" href="mailto:info@webistrydesk.com">info@webistrydesk.com</a> or{" "}
              <a className="underline" href="mailto:support@webistrydesk.com">support@webistrydesk.com</a>
            </p>
          </form>
        </div>
      </section>
