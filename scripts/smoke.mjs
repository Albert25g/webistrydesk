...
try {
  ...
  if (response.statusCode >= 200 && response.statusCode < 400) {
    ...
    if (response.body.includes('WebistryDesk') || response.body.includes('webistrydesk')) {
      console.log('✅ Content check PASSED - WebistryDesk branding found');
    } else {
      console.log('⚠️  Content check WARNING - WebistryDesk branding not found in response');
    }
